...
document.getElementById("sendAssistantBtn").addEventListener("click", async () => {
    ...
    const result = await fetchAssistant(userMessage, assistantPrompt);
...
async function fetchGPT(userInput, systemPrompt) {
  ...
  async function fetchAssistant(userInput, customPrompt) {
    try {
      const headers = {
        Authorization: `Bearer ${API_KEY}`,
        "Content-Type": "application/json",
      ...
      const runRes = await fetch(`https://api.openai.com/v1/threads/${threadId}/runs`, {
        ...
        //   instructions: customPrompt || "당신은 친절한 조수입니다."
        })
      });
      ...
      while (status !== "completed") {
        ...
        if (status === "failed") {
          throw new Error("⚠️ Assistant 처리 실패");
        }
      }
      ...
      if (!msgRes.ok) {
        throw new Error(`❌ 메시지 가져오기 실패: ${msgRes.status}`);
      }
  
      const msgData = await msgRes.json();
      const lastMsg = msgData.data.find(m => m.role === "assistant");
  
      return lastMsg?.content[0]?.text?.value || "(응답 없음)";
    } catch (err) {
      console.error(err);
      return `⚠️ 오류: ${err.message}`;
    }
